...
from .builder import build_prototype_aggregation#加
from sklearn.metrics.pairwise import cosine_similarity#加
...
class FPDRoIHead(StandardRoIHead):
    ...
    def __init__(self,
                 ...
                 meta_cls_ratio=1.0,
                 prototype_aggregation: Optional[ConfigDict] = None,#加
                 **kwargs) -> None:
        ...
        if prototype_aggregation is not None:
            self.prototype_aggregation = build_prototype_aggregation(copy.deepcopy(prototype_aggregation))
        #加  
    ...
    def _optimized_bbox_forward_train(self, query_feats: List[Tensor],
        ...
        #fused_feats = self.prototypes_assignment(fused_feats[0], prototypes)
        # ************ POST RPN *****************
        if self.with_rpn:
            if self.with_rpn:
        ...
        support_feat_change = self.extract_support_feats_without_mean(support_feats)
        ...
        for k in range(num_supp_per_im):
            ...
            #
            #random_index = np.random.choice(
            #    range(query_gt_labels[k].size(0)))
            #random_query_label = query_gt_labels[k][random_index]
            #support_feat_change = support_feat_change[k]
            #support_feat, support_gt_labels = self.prototype_aggregation(support_feat_change, support_gt_labels_)
            #rpt_prototype = support_feat[supp_order]
            #加
    ...
    def extract_support_feats_without_mean(self, feats: List[Tensor]) -> List[Tensor]:
        out = []
        if self.with_shared_head:
            for lvl in range(len(feats)):
                out.append(self.shared_head.forward_support_without_mean(feats[lvl]))
        else:
            out = feats
        return out
    ...
    def simple_test_bboxes(
        ...
        #fused_feats = self.prototypes_assignment(fused_feats[0], prototypes, query_img_metas)#加
