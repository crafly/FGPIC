...
def main():
    ...
    def draw_boxes(img_path, boxes, labels, scores, output_path, class_names, score_thr=0.3):
    ...
    if rank == 0:
        ...
        if args.show_dir:
            for i, (result, img_meta) in enumerate(zip(outputs, dataset.data_infos)):
                boxes = np.concatenate([r[:, :4] for r in result], axis=0)
                scores = np.concatenate([r[:, 4] for r in result], axis=0)
                labels = np.concatenate([[cls_id] * len(r) for cls_id, r in enumerate(result)], axis=0)
                img_path = img_meta['filename']
                output_path = os.path.join(args.show_dir, os.path.basename(img_path))
                draw_boxes(img_path, boxes, labels, scores, output_path, model.CLASSES, score_thr=args.show_score_thr)
